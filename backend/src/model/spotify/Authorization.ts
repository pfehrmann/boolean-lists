...
export async function getApiFromUser(
  user: IUser
): Promise<InitializedSpotifyApi> {
  if (
    user &&
    user.authorization &&
    user.authorization.accessToken &&
    user.authorization.refreshToken
  ) {
    ...
    if (Date.now() > user.authorization.expiresAt) {
      ...
      logger.debug(
        `Not refreshing token, valid util ${user.authorization.expiresAt}`
      );
    }
...
function userToSpotifyApi(user: IUser) {
...
async function refreshCredentials(user: any) {
  ...
  const refreshToken =
    response.body.refresh_token || user.authorization.refreshToken;
  user.authorization = {
